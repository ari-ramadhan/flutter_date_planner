...
class Event implements Comparable<Event> {
  ...
  bool get isToday => DateTime.now().isSameDay(date) && ((DateTime.now().hour < date.hour) && (DateTime.now().minute < date.minute));

  bool get isTomorrow =>
      DateTime.now().add(const Duration(days: 1)).isSameDay(date);

  bool get isComplete => remainingTaskCount == 0;

  bool get isPast => DateTime.now().isAfter(date);

  bool get isWithinSevenDays => !isTomorrow && !isPast && date.isBefore(FromNow.sevenDays);
